...
class _FilmSayfasiState extends State<FilmSayfasi> {
  ...
  List<Map<String, dynamic>> benzerFilmler = [];
  ...
  Future<void> loadFilmDurum() async {
    final res = await http.get(
      Uri.parse(
        "$apiBase/api/film/durum?kullaniciId=${widget.benimID}&filmId=${widget.film["id"]}",
      ),
  ...
  Future<void> izleToggle() async {
    final res = await http.post(
      Uri.parse("$apiBase/api/film/izlendi"),
      headers: {
      ...
      body: jsonEncode({
        ...
        "filmAdi": widget.film["title"] ?? "",
        "posterPath": widget.film["poster_path"] ?? "",
      }),
    );
    if (res.statusCode == 200) setState(() => izlendi = !izlendi);
  }

  Future<void> dahaSonraToggle() async {
    final res = await http.post(
      Uri.parse("$apiBase/api/film/dahasonra"),
      headers: {
        "Content-Type": "application/json",
        "ngrok-skip-browser-warning": "true",
      },
    ...
    if (res.statusCode == 200) setState(() => dahaSonra = !dahaSonra);
  }

  Future<void> favoriToggle() async {
    final res = await http.post(
      Uri.parse("$apiBase/api/film/favori"),
      headers: {
        "Content-Type": "application/json",
        "ngrok-skip-browser-warning": "true",
      },
    ...
    if (res.statusCode == 200) setState(() => favori = !favori);
  }
  ...
  Future<void> puanVer(int yeniPuan) async {
    ...
    await http.put(
      Uri.parse(
        "$apiBase/api/film/puan?kullaniciId=${widget.benimID}&filmId=${widget.film["id"]}&puan=$yeniPuan",
      ),
      headers: {"ngrok-skip-browser-warning": "true"},
    );
  ...
  Future<void> loadDetay() async {
    final res = await http.get(
      Uri.parse(
        "https://api.themoviedb.org/3/movie/${widget.film["id"]}?api_key=$tmdbKey&language=tr-TR&append_to_response=credits",
      ),
    );
    if (res.statusCode == 200) {
      final d = jsonDecode(res.body);
  ...
  Future<void> loadBenzer() async {
    final res = await http.get(
      Uri.parse(
        "https://api.themoviedb.org/3/movie/${widget.film["id"]}/similar?api_key=$tmdbKey&language=tr-TR",
      ),
    );
    if (res.statusCode == 200) {
      final d = jsonDecode(res.body);
  ...
  Future<void> loadElestiriler() async {
    final res = await http.get(
      Uri.parse("$apiBase/api/filmelestiri/film-detay/${widget.film["id"]}"),
      headers: {"ngrok-skip-browser-warning": "true"},
    );
    if (res.statusCode == 200) {
  ...
  Future<void> elestiriBegen(int elestiriId) async {
    setState(() {
      for (var e in elestiriler) {
        if ((e["id"] ?? e["ID"]) == elestiriId) {
          if (e["begeniSayisi"] != null)
            e["begeniSayisi"]++;
          else if (e["BegeniSayisi"] != null)
            e["BegeniSayisi"]++;
          else
            e["begeniSayisi"] = 1;
          break;
    ...
    try {
      await http.put(
        Uri.parse(
          "$apiBase/api/filmelestiri/begen?elestiriId=$elestiriId&kullaniciId=${widget.benimID}",
        ),
        headers: {"ngrok-skip-browser-warning": "true"},
      );
  ...
  Future<void> elestiriEkle() async {
    ...
    await http.post(
      Uri.parse("$apiBase/api/filmelestiri"),
      headers: {
        "Content-Type": "application/json",
        "ngrok-skip-browser-warning": "true",
      },
  ...
  Future<void> yanitEkle(int anaId) async {
    ...
    await http.post(
      Uri.parse("$apiBase/api/elestiriyanit"),
      headers: {
        "Content-Type": "application/json",
        "ngrok-skip-browser-warning": "true",
      },
      ...
      body: ListView(
        ...
        children: [
          Row(
            ...
            children: [
              ClipRRect(
                ...
                child: Image.network(
                  "$tmdbImg${widget.film["poster_path"]}",
                  width: 140,
                  height: 210,
                  fit: BoxFit.cover,
                ),
          ...
          GridView.builder(
            ...
            itemCount: benzerFilmler.length,
            ...
            itemBuilder:
                (_, i) => ClipRRect(
                  ...
                  child: Image.network(
                    "$tmdbImg${benzerFilmler[i]["poster_path"]}",
                    fit: BoxFit.cover,
                  ),
                ),
          ),
          ...
          TextField(
            ...
            decoration: InputDecoration(
              hintText: "Film hakkında ne düşünüyorsun?",
  ...
  Widget elestiriKarti(Map<String, dynamic> e) {
    ...
    final int begeniSayisi = e["begeniSayisi"] ?? e["BegeniSayisi"] ?? 0;
